...
//seria queue thread safe becouse of blocke data in serial
    var dispatchQueue = DispatchQueue.init(label: "stackQueu")
    func push0(_ item: T) {
        dispatchQueue.async {
             self.items.append(item)
        }
...
// concorent barier sync read / async barie write
